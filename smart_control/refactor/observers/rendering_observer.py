...
from smart_control.refactor.observers.base_observer import Observer
...
class RenderingObserver(Observer):
    ...
    def __init__(
        ...
        Args:
            ...
            plot_fn: Optional function to use for plotting. If not provided,
                no plotting will be done.
    ...
    def _format_plot(
        ...
        ax1.xaxis.set_major_formatter(mdates.DateFormatter('%a %m/%d %H:%M', tz=pytz.timezone(time_zone)))
    ...
    def _plot_action_timeline(self, ax1, action_timeseries, action_tuple, time_zone):
        ...
        if action_tuple[1] in ['supply_water_setpoint','supply_air_heating_temperature_setpoint']:
            single_action_timeseries['setpoint_value'] = (single_action_timeseries['setpoint_value'] - self.KELVIN_TO_CELSIUS)
    ...
    def _plot_timeseries_charts(self, reader, time_zone, step_count):
        ...
        fig, axes = plt.subplots(
            ...
            ncols=1,
            gridspec_kw={'height_ratios': [1, 1, 1, 1, 1, 1] + [1] * len(action_tuples)},
            squeeze=True,
        )
        ...
        for i, action_tuple in enumerate(action_tuples):
            self._plot_action_timeline(axes[6 + i], action_timeseries, action_tuple, time_zone)
