...
TIMESTAMP = pd.Timestamp('2022-03-13 00:00:00', tz='US/Pacific')
...
class ObservationResponseParserTest(absltest.TestCase):
  ...
  def test_timestamp(self):
    self.assertEqual(self.parser.timestamp, TIMESTAMP)

  def test_get_local_time(self):
    with self.subTest(name='default time zone'):
      self.assertEqual(self.parser.get_local_time(), TIMESTAMP)

    with self.subTest(name='different time zone'):
      self.assertEqual(
          self.parser.get_local_time('US/Eastern'),
          TIMESTAMP.tz_convert('US/Eastern')
      )
  ...
  def test_outside_air_temp(self):
    ...
    with self.subTest(name='has outside air temp'):
      response = test_utils.get_test_observation_response(
          timestamp=TIMESTAMP,
          device_measurement_values=[
              ('device_0', 'measurement_0', 7.0),
              ('device_0', 'measurement_1', 0.1),
