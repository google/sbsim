...
def modify_config(config_content, param_name, param_value):
    ...
    modified_content = re.sub(pattern, replacement, config_content, flags=re.MULTILINE)
    
    if modified_content == config_content:
        logger.warning(f"Warning: Parameter '{param_name}' not found in config file.")
...
def main():
    parser = argparse.ArgumentParser(description='Generate grid of gin config files')
    parser.add_argument('base_config', help='Path to the base gin config file')
    parser.add_argument('--output-dir', default='generated_configs', 
                        help='Directory to save generated config files')
    parser.add_argument('--time-steps', type=str, default='300',
                        help='Comma-separated list of time_step_sec values')
    parser.add_argument('--num-days', type=str, default='1,7,14,30',
                        help='Comma-separated list of num_days_in_episode values')
    parser.add_argument('--start-timestamps', type=str, default='2023-07-06',
    ...
    start_timestamps = [f"'{ timestamp.strip() } 07:00:00+00:00'" for timestamp in args.start_timestamps.split(',')]
    ...
    logger.info(f"Generated {len(time_steps) * len(num_days)} configuration files in {args.output_dir}")
