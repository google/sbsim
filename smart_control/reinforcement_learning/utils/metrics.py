...
def get_trajectory(time_step: ts.TimeStep, current_action: policy_step.PolicyStep) -> trajectory.Trajectory:
...
def compute_avg_return(environment: Any,
    ...
    Returns:
        Tuple of (average return, list of [simulation time, episode return] pairs).
    """
    total_return = 0.0
    return_by_simtime = []
    ...
    for _ in range(num_episodes):
        ...
        for _ in range(num_steps):
            ...
            sim_time = environment.pyenv.envs[0].current_simulation_timestamp.tz_convert(time_zone)

            return_by_simtime.append([sim_time, episode_return])

            logger.info("[Step %d] [Sim Time: %s] [Reward: %.2f] [Return: %.2f] [Mean Step Time: %.2fs] [Episode Time: %.2fs]"
                % (step_id, sim_time.strftime("%Y-%m-%d %H:%M"), time_step.reward, episode_return, np.mean(execution_times), episode_seconds))
