...
class MultiEpisodeWrapper(py_environment.PyEnvironment):
    ...
    def __init__(self, scenario_config_paths: collections.abc.Sequence[str], create_env_fn: collections.abc.Callable):
        ...
        logger.info(f"Initializing LazyMultiScenarioPyEnvironment with {len(scenario_config_paths)} config paths.")
        ...
        try:
            ...
            logger.info("Successfully loaded initial environment and determined specs.")
        except Exception as e:
            logger.exception(f"Failed to load or reset the initial environment "
    ...
    def _load_and_reset_env(self):
        ...
        logger.info(f"Loading environment from config: {config_path} (index {self._current_env_index})")
        ...
        try:
            ...
            if not isinstance(self._current_env, py_environment.PyEnvironment):
                 raise TypeError(f"create_env_fn did not return a PyEnvironment instance for path: {config_path}")

            # Reset the newly loaded environment
            self._state = self._current_env.reset()
            logger.debug(f"Successfully loaded and reset environment index {self._current_env_index}")

        except Exception as e:
            logger.exception(f"Failed to load or reset environment from config path: {config_path}")
            # Propagate the error to indicate failure
            raise RuntimeError(f"Failed to load/reset environment from {config_path}") from e
    ...
    def current_config_path(self) -> str | None:
        ...
        if self._current_env_index >= 0 and self._current_env_index < self._num_paths:
    ...
    def _reset(self) -> ts.TimeStep:
        ...
        logger.debug(f"Reset called. Closing current environment (index {self._current_env_index})...")
        ...
        if self._current_env is not None:
            try:
                ...
                logger.error(f"Error closing environment from path "
    ...
    def _step(self, action) -> ts.TimeStep:
        ...
        if self._current_env is None:
            ...
            raise RuntimeError("Step called but no environment is currently loaded. Was reset() called?")
        ...
        if self._state.is_last():
            logger.debug(f"Episode ended in environment index: {self._current_env_index}. "
                         f"Next reset call will switch environment.")
    ...
    def close(self):
        ...
        if self._current_env is not None:
            try:
                logger.info(f"Closing currently active environment (index {self._current_env_index}, "
                            f"path: {self.current_config_path})")
                self._current_env.close()
            except Exception as e:
                logger.error(f"Error closing environment from path "
                             f"'{self.current_config_path}': {e}")
    ...
    def render(self, mode='rgb_array'):
        ...
        try:
            ...
            logger.error(f"Failed to render environment index {self._current_env_index}: {e}")
