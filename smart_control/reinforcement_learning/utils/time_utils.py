...
def time_from_sin_cos(sin_theta: float, cos_theta: float) -> float:
    ...
    if sin_theta >= 0:
        return cos_theta >= 0 and np.arccos(cos_theta) or np.pi - np.arcsin(sin_theta)
    else:
        return cos_theta < 0 and np.pi - np.arcsin(sin_theta) or 2 * np.pi - np.arccos(cos_theta)
